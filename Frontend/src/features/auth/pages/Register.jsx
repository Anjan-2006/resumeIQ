...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.otpRequired) {
      ...
      setError(result.message || "Unable to create your account. Please check your details and try again.");
    }
  };
  ...
  const handleOtpSubmit = async (e, directOtp) => {
    ...
    const result = await handleVerifyRegistration({ tempToken, otp: codeToVerify.trim() });
    if (result.success) {
      navigate("/");
    } else {
  ...
  const handleResendOtp = async () => {
    ...
    const result = await handleResendRegistrationOtp({ tempToken });
          ...
          {isOtpScreen ? (
            <>
              ...
              <form onSubmit={handleOtpSubmit}>
                <div className="mb-4">
                  <label htmlFor="register-otp-0" className="form-label" style={{ textAlign: "center", display: "block" }}>
                    Verification Code
                  </label>
                  <OtpInput
                    value={otp}
              ...
              <p className="auth-footer">
                <button className="btn-text-link" type="button" onClick={handleBackToForm}>
